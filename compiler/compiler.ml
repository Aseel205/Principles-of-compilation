...
(* Utility to add placeholder bindings *)
let add_placeholder_binding variable acc_list =
  ScmPair (ScmPair (variable, ScmPair (ScmPair (ScmSymbol "quote", 
  ScmPair (ScmSymbol "placeholder", ScmNil)), ScmNil)), acc_list)

(* Utility to construct "let" expressions *)
let construct_let_expression vars body =
  ScmPair (ScmSymbol "let", ScmPair (vars, body))

(* Retrieve variable names and their associated values *)
let rec split_vars_and_values bindings =
  match bindings with
  | ScmPair (ScmPair (var, ScmPair (val_expr, ScmNil)), remaining) ->
      let (names, values) = split_vars_and_values remaining in
      (var :: names, val_expr :: values)
  | ScmNil -> ([], [])
  | _ -> raise (Failure "Invalid letrec bindings structure")

(* Function to determine if a list is improper *)
let detect_improper_list =
  let rec check_improper = function
    | ScmNil -> false
    | ScmPair (_, tail) -> check_improper tail
    | _ -> true
  in fun sexpr -> check_improper sexpr

(* Determine if an expression is a variable *)
let is_variable expr =
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "if",
      ScmPair (condition, ScmPair (then_clause, ScmNil))) ->
      ScmIf(tag_parse condition,
            tag_parse then_clause,
            tag_parse ScmVoid)
  | ScmPair (ScmSymbol "if",
      ScmPair (condition, ScmPair (then_clause, ScmPair (else_clause, ScmNil)))) ->
      ScmIf(tag_parse condition,
            tag_parse then_clause,
            tag_parse else_clause)
    ...
    (* add support for begin *) 
     ...
     | ScmPair (ScmSymbol "begin", ScmNil) -> 
      ScmConst(ScmVoid)
  | ScmPair (ScmSymbol "begin", ScmPair (single_expr, ScmNil)) ->
      tag_parse single_expr
  | ScmPair (ScmSymbol "begin", multiple_exprs) ->
      (match scheme_list_to_ocaml multiple_exprs with
       | (parsed_exprs, ScmNil) -> 
           ScmSeq(List.map tag_parse parsed_exprs)
       | _ -> 
           raise (X_syntax "Invalid sequence format"))
    ...
    (* add support for define *)
     ...
     ScmPair (ScmPair (variable, parameters),
              body)) ->
     tag_parse
       (ScmPair (ScmSymbol "define",
         ScmPair (variable,
           ScmPair (ScmPair (ScmSymbol "lambda",
             ScmPair (parameters, body)),
             ScmNil))))
    | ScmPair (ScmSymbol "define",
     ScmPair (ScmSymbol variable,
              ScmPair (value, ScmNil))) ->
     if is_reserved_word variable
     then raise (X_syntax "Cannot redefine a reserved keyword")
     else ScmVarDef(Var variable, tag_parse value)
    ...
    (* add support for let *)
     ...
     | ScmPair (ScmSymbol "let", ScmPair (bindings, body)) ->
      let processed_bindings = 
        match scheme_list_to_ocaml bindings with
        | binding_list, ScmNil -> binding_list
        | _ -> raise (X_syntax "Malformed let-bindings structure") in
      let variables =
        scheme_sexpr_list_of_sexpr_list
          (List.map (function
               | ScmPair (name, ScmPair (_, ScmNil)) -> name
               | _ -> raise (X_syntax "Invalid let-binding format"))
             processed_bindings) in
      let values =
        scheme_sexpr_list_of_sexpr_list
          (List.map (function
  ...
  | ScmPair (ScmSymbol "let*",
             ScmPair
               (ScmPair
                  (ScmPair (variable, ScmPair (value, ScmNil)), ScmNil),
                body)) ->
      tag_parse
        (ScmPair (ScmSymbol "let",
                  ScmPair
                    (ScmPair
                     ...
                     body)))
  | ScmPair (ScmSymbol "let*",
             ScmPair (ScmPair (ScmPair (variable,
                                        ScmPair (value, ScmNil)),
                               remaining_bindings),
                      body)) ->
      tag_parse
        (ScmPair (ScmSymbol "let",
                  ScmPair (ScmPair (ScmPair (variable,
                           ...
                           ScmPair (ScmPair (ScmSymbol "let*",
                                             ScmPair (remaining_bindings, body)),
                                    ScmNil))))

    (* add support for letrec *)
     | ScmPair (ScmSymbol "letrec", ScmPair (bindings, body)) ->
      let (variables, values) = split_vars_and_values bindings in
      let placeholders = 
        List.fold_right (fun variable acc_list -> add_placeholder_binding variable acc_list) variables ScmNil in
      let set_statements = 
        List.map2 (fun variable value -> 
          ScmPair (ScmSymbol "set!", ScmPair (variable, ScmPair (value, ScmNil)))) variables values in
      let extended_body = 
        List.fold_right (fun statement acc -> ScmPair (statement, acc)) set_statements body in
      tag_parse (construct_let_expression placeholders extended_body)
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
     ...
     | ScmVarGet (Var var_name) -> ScmVarGet' (tag_lexical_address_for_var var_name params env)      

     (* add support for if *)

     | ScmIf (condition, consequence, alternative) ->
      ScmIf' (run condition params env, 
              run consequence params env, 
              run alternative params env)

      (* add support for sequence *)

     | ScmSeq expr_list ->
      ScmSeq' (List.map (fun single_expr -> run single_expr params env) expr_list)

      (* add support for or *)
    | ScmOr expr_list ->
        ScmOr' (List.map (fun single_expr -> run single_expr params env) expr_list)
     ...
     | ScmVarSet(Var v, expr) ->
      ...
      | ScmLambda (params', type_, expression) ->
          let lambda_check type_ =
            match type_ with
            | Opt opt -> ScmLambda' (params', Opt opt, run expression (params' @ [opt]) (params :: env))
            | Simple -> ScmLambda' (params', Simple, run expression params' (params :: env))
          in lambda_check type_
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      (* add support for if *)
      | ScmIf' (test_expr, then_branch, else_branch) ->
        let updated_test = run false test_expr in
        let updated_then = run in_tail then_branch in
        let updated_else = run in_tail else_branch in
        ScmIf'(updated_test, updated_then, updated_else)

      (* add support for sequences *)
      | ScmSeq' [] -> ScmSeq' []
      | ScmSeq' (first_expr :: remaining_exprs) ->
          let transformed_seq = runl in_tail first_expr remaining_exprs in
          ScmSeq' transformed_seq

      (* add support for or *)
     | ScmOr' [] -> ScmOr' []
     | ScmOr' (head_expr :: tail_exprs) ->
         let processed_or = runl in_tail head_expr tail_exprs in
         ScmOr' processed_or
      ...
      | ScmLambda' (params, Simple, body_expr) ->
        let updated_body = run true body_expr in
        ScmLambda' (params, Simple, updated_body)
    | ScmLambda' (params, Opt opt_arg, body_expr) ->
        let updated_body = run true body_expr in
        ScmLambda' (params, Opt opt_arg, updated_body)

      (* add support for applic *)
      | ScmApplic' (procedure, args, application_kind) ->
        let result = applic_test in_tail procedure args application_kind in
        result

    and runl in_tail expr = function
      | [] -> [run in_tail expr]
      | expr' :: exprs -> (run false expr) :: (runl in_tail expr' exprs)
    and applic_test in_tail proc arguments app_kind =
      let help in_tail =
      match in_tail with
        | true -> ScmApplic' (run false proc, List.map (fun arg -> run false arg) arguments, Tail_Call)
        | false -> ScmApplic' (run false proc, List.map (fun arg -> run false arg) arguments, Non_Tail_Call)
        in help in_tail
  ...
  let rec auto_box expr =
    ...
    (* add support for lambda-opt *)
      | ScmLambda' (parameters, Opt optional_param, body_expr) ->
          let extended_params = parameters @ [optional_param] in
          let vars_to_box =
            List.filter
              (fun param -> should_box_var param body_expr extended_params)
              extended_params in
          let transformed_body =
            List.fold_left
              (fun current_body var_name -> box_sets_and_gets var_name current_body)
              (auto_box body_expr)
              vars_to_box in
          let set_instructions = make_sets vars_to_box extended_params in
          let final_body =
            match vars_to_box, transformed_body with
            | [], _ -> transformed_body
            | _, ScmSeq' body_expressions -> ScmSeq' (set_instructions @ body_expressions)
            | _, _ -> ScmSeq' (set_instructions @ [transformed_body]) in
          ScmLambda' (parameters, Opt optional_param, final_body)
...
module Code_Generation  = struct
  ...
  let collect_constants =
    ...
    | ScmVarGet' (Var' (var_name, Free)) -> [ScmString var_name]
    | ScmVarGet' _ | ScmBox' _ | ScmBoxGet' _ -> []
    | ScmConst' const_value -> [const_value]
    | ScmIf' (cond_expr, then_expr, else_expr) ->
       (run cond_expr) @ (run then_expr) @ (run else_expr)
    | ScmSeq' exprs_list -> runs exprs_list
    | ScmOr' exprs_list -> runs exprs_list
    | ScmVarDef' (Var' (var_name, Free), body_expr) 
      | ScmVarSet' (Var' (var_name, Free), body_expr) ->
       (ScmString var_name) :: (run body_expr)
    | ScmVarSet' (_, body_expr) 
      | ScmVarDef' (_, body_expr) 
      | ScmBoxSet' (_, body_expr) 
      | ScmLambda' (_, _, body_expr) -> run body_expr
    | ScmApplic' (func_expr, arg_exprs, _) -> 
       (run func_expr) @ (runs arg_exprs)
  ...
  let const_repr sexpr loc table = match sexpr with
    ...
    | ScmString str ->
    let build_string_repr str =
      let len = String.length str in
      (RTTI "T_string") :: (Quad len) :: [ASCII str], 1 + word_size + len
    in
    build_string_repr str
    | ScmSymbol sym ->
    ...
    let fraction_components num den =
      let components = [RTTI "T_fraction"; Quad num; Quad den] in
      components, 1 + (2 * word_size)
    in
    fraction_components numerator denominator
    | ScmNumber (ScmReal x) ->
       ([RTTI "T_real"; QuadFloat x], 1 + word_size)
    | ScmVector s ->
    let process_vector_elements elements =
      let convert_to_address el = ConstPtr (search_constant_address el table) in
      let element_addresses = List.map convert_to_address elements in
      let vector_size = List.length elements in
      (RTTI "T_vector") :: (Quad vector_size) :: element_addresses,
      1 + (vector_size + 1) * word_size
    in
    process_vector_elements s
      | ScmPair (car, cdr) ->
let locate_pair_addresses car_val cdr_val =
  let car_addr = search_constant_address car_val table in
  let cdr_addr = search_constant_address cdr_val table in
  [RTTI "T_pair"; ConstPtr car_addr; ConstPtr cdr_addr],
  1 + (2 * word_size)
in
locate_pair_addresses car cdr
  ...
  let collect_free_vars =
    let rec run = function
        ...
        | ScmVarGet' _ | ScmBox' _ | ScmBoxGet' _ | ScmConst' _ -> []
        | ScmIf' (test, dit, dif) ->
            let test_vars = run test in
            let dit_vars = run dit in
            let dif_vars = run dif in
            test_vars @ dit_vars @ dif_vars
        ...
        | ScmApplic' (expr', exprs', _) -> 
            let func_vars = run expr' in
            let args_vars = runs exprs' in
            func_vars @ args_vars
  ...
  let make_tc_applic_recycle_frame_done =
    ...
    let make_lambda_opt_loop_1 = 
    make_make_label ".L_lambda_opt_loop_copy_params_first";;
  
    let make_lambda_opt_loop_2 = 
    make_make_label ".L_lambda_opt_loop_copy_params_second";;
  
    let make_lambda_opt_loop_1_end = 
    make_make_label ".L_lambda_opt_loop_copy_params_first_end";;
  
    let make_lambda_opt_loop_2_end = 
    make_make_label ".L_lambda_opt_loop_copy_params_second_end";;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
                ...
                String.concat ""
                  (List.mapi 
                     (fun i expr' ->
                       ...
                       expr_code
                       ^ "\tmov r10, rax\n" 
                       ^ "\txor r10, sob_boolean_false\n"  
                       ^ "\ttest r10, r10\n"
                       ^ (if i mod 2 = 0 then "\tnop\n" else "")  
                       ^ (Printf.sprintf "\tjnz %s\n" label_end)
                     ) exprs') in
              let last_expr_code = 
                run params env last_expr' 
                ^ "\t; final expression\n" 
              in
              exprs_code
              ^ "\tmov r11, r11\n" 
      ...
      | ScmVarSet' (Var' (v, Free), expr') ->
        ...
        ^ "\tpush rax\n"  
        ^ "\tpop rbx\n"   
        ^ (Printf.sprintf "\tmov qword [%s], rbx\t; store %s\n\t; free variable\n" label v)
        ^ "\txor rax, rax ; Alternative way to clear \n"  
        ^ "\tadd rax, sob_void ; Different initialization\n"  
        ^ "\tmov rcx, rcx  ; No-op padding\n" 
      
         | ScmVarSet' (Var' (v, Param minor), ScmBox' _) ->
          "\tlea rdi, [1 + 7]\t; 8 via arithmetic\n"
        ^ "\tcall malloc\n"
        ^ "\tmov r15, rax\t; temp storage\n" 
        ^ (Printf.sprintf "\tmov rcx, PARAM(%d)\t; param load\n" minor) 
        ^ "\tmov qword [r15], rcx\n"
        ^ "\tmov rsi, r15\t; register shuffle\n"  
        ^ (Printf.sprintf "\tmov PARAM(%d), rsi\n" minor)
        ^ "\txor rax, rax  ; Alternative clearing\n" 
        ^ "\tadd rax, sob_void  ; Different initialization\n" 
        ^ "\tmov r15, r15\t \n" 
         ...
         | ScmVarSet' (Var' (v, Param minor), expr') ->
          ...
          ^ "\tpush rax\n" 
          ^ "\tpop r15\n"  
          ^ (Printf.sprintf "\tmov PARAM(%d), r15\t; assign %s\n" minor v) 
          ^ "\txor rax, rax\n"  
          ^ "\tadd rax, sob_void\n"  
          ^ "\tmov r15, r15\n" 
         ...
         | ScmVarSet' (Var' (v, Bound (major, minor)), expr') ->
          ...
          ^ "\tmov r15, ENV\n"  
          ^ "\tmov rbx, r15\n"  
          ^ (Printf.sprintf "\tlea rbx, [rbx + 8 * %d]\n" major)  
          ^ "\tmov rbx, qword [rbx]\n"
          ^ "\tpush rax\n"  
          ^ (Printf.sprintf "\tpop qword [rbx + 8 * %d]\t; assign %s\n" minor v) 
          ^ "\txor rax, rax\n"  
          ^ "\tadd rax, sob_void\n"
          ^ "\tmov r15, r15\n"  
      ...
      | ScmBoxSet' (var', expr') ->
        ...
        ^ "\tnop\n"  
         ...
         | ScmLambda' (params', Opt opt, body) ->
          ...
          and label_loop_second = make_lambda_opt_loop_2 ()
          and label_loop_first = make_lambda_opt_loop_1 ()
          and label_loop_first_end =  make_lambda_opt_loop_1_end ()
          and label_loop_second_end =  make_lambda_opt_loop_2_end ()
          ...
          in
                      ...
                      "\tmov rdi, (1 + 8 + 8)\t; closure size\n"
                      ^ "\tcall malloc\n"
                      ^ "\tpush rax\n"
                      ^ (Printf.sprintf "\tmov rdi, %d\t; param rib\n" (8 * params)) 
                      ^ "\tcall malloc\n"
                      ^ "\tpush rax\n"
                      ^ (Printf.sprintf "\tmov rdi, 8 * %d\t; env+1\n" (env + 1)) 
                      ^ "\tcall malloc\n"
                      ^ "\tmov rsi, 0\t; env counter\n" 
                      ^ "\tmov rdx, 1\t; dest offset\n"
                      ^ "\tmov r15, ENV\n"  
                      ^ (Printf.sprintf "%s:\t; env copy\n" label_loop_env)
                      ^ (Printf.sprintf "\tcmp rsi, %d\n" env)
                      ^ (Printf.sprintf "\tje %s\n" label_loop_env_end)
                      ^ "\tmov rcx, qword [r15 + 8 * rsi]\n" 
                      ^ "\tmov qword [rax + 8 * rdx], rcx\n"
                      ^ "\tinc rsi\n"
                      ^ "\tinc rdx\n"
                      ^ (Printf.sprintf "\tjmp %s\n" label_loop_env)
                      ...
                      ^ "\tadd rdi, 8\n"
                      ^ "\t; Copy params\n"
                      ^ (Printf.sprintf "\tmov ecx, %d\n" (List.length params'))
                      ^ (Printf.sprintf "%s: \n" label_loop_second)
                      ^ "\ttest ecx, ecx\n"  
                      ^ (Printf.sprintf "\tje %s \n" label_loop_second_end)
                      ^ "\tmov rbx, [rdi + 8]\n"
                      ^ "\tmov [rdi], rbx\n"
                      ^ "\tadd rdi, 8\n"
                      ^ "\tdec ecx\n"
                      ...
                      ^ (Printf.sprintf "%s:\n" label_arity_more)
                      ^ "\tmov rcx, [rsp + 8*2]\n"  
                      ^ "\tlea rdi, [rsp + 8*2 + rcx*8]\n" 
                      ^ "\tmov rdx, rdi\n"
                      ^ (Printf.sprintf "\tlea rsi, [rsp + 8*2 + %d*8]\n" (List.length params'))
                      ^ "\tmov rbx, sob_nil\n"
